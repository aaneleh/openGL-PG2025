...
TriangleV2 setupStruct(float x1, float y1, float x2, float y2, float x3, float y3, float r, float g, float b);
...
int main(){
	...
	while (!glfwWindowShouldClose(window)){
        ...
        if (state == GLFW_PRESS) {
			...
			triangle.VAO = setupGeometry((dx/500)-0.1, (dy/500)-0.1, (dx/500)+0.1, (dy/500)-0.1, (dx/500)+0.0, (dy/500)+0.1);
...
TriangleV2 setupStruct(float x1, float y1, float x2, float y2, float x3, float y3, float r, float g, float b){
	TriangleV2 triangle;
	triangle.position = vec3(x1, y1, 0);
	triangle.color = vec3(r,g,b);
	triangle.VAO = setupGeometry(x1, y1, x2, y2, x3, y3);
	return triangle;
}
